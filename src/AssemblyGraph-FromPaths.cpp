...
// findAssemblyPaths.
void AssemblyGraph::connectAssemblyPaths(
    ...
    vector< std::list<edge_descriptor> >& linearChains)
{
	...
	for(const vector<edge_descriptor>& assemblyPath: assemblyPaths) {
		...
		// Fill in the linear chain generated by this assembly path.
		auto& linearChain = linearChains.emplace_back();
		for(uint64_t i0=0; i0<newAssemblyPath.size(); i0++) {
            const edge_descriptor e0 = newAssemblyPath[i0];
		    linearChain.push_back(e0);

		    // If not the last, add the next edge.
		    if(i0 != newAssemblyPath.size() - 1) {
		        const uint64_t i1 = i0 + 1;
		        const edge_descriptor e1 = newAssemblyPath[i1];
		        const vertex_descriptor v0 = target(e0, assemblyGraph);
		        const vertex_descriptor v1 = source(e1, assemblyGraph);
		        auto[eNew, edgeExists] = boost::edge(v0, v1, assemblyGraph);
		        SHASTA2_ASSERT(edgeExists);
		        linearChain.push_back(eNew);
		    }
		}
	}
