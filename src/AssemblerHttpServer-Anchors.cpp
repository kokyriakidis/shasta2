...
void Assembler::exploreRestrictedAnchorGraph(
    const vector<string>& request,
    ostream& html)
{
    // Get the request parameters.
    ...
    uint64_t distanceInJourney = 10;
    HttpServer::getParameterValue(request, "distanceInJourney", distanceInJourney);
    ...
    // Distance in journey.
    html << "<tr>"
        "<th class=left>Distance in journey"
        "<td class=centered>"
        "<input type=text name=distanceInJourney style='text-align:center' required size=8 value=" <<
        distanceInJourney << ">";
    ...
    html << "<h2>Local journey analysis</h2>";

    // Create the RestrictedAnchorGraph.
    RestrictedAnchorGraph restrictedAnchorGraph(anchors(), journeys(),
        anchorId, uint32_t(distanceInJourney), html);
    restrictedAnchorGraph.approximateTopologicalSort();
    restrictedAnchorGraph.writeOrientedReadsInVertices(html);

    // Write it out in Graphviz format.
    const string uuid = to_string(boost::uuids::random_generator()());
    const string dotFileName = tmpDirectory() + uuid + ".dot";
    restrictedAnchorGraph.writeGraphviz(dotFileName, {anchorId});
